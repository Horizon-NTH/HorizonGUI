...
namespace hgui
{
	...
	{
	public:
		static const std::shared_ptr<hgui::kernel::Button>& create(const std::string& buttonName, std::function<void()> function, glm::vec2 size, glm::vec2 position, std::shared_ptr<kernel::Texture> texture, glm::vec3 color = glm::vec3(1.0f), const std::string& text = "", const std::shared_ptr<kernel::Font>& font = nullptr, std::tuple<unsigned int, glm::vec3, float> textOptions = { 12u, glm::vec3(1.0), 1.0f }, float angularRotation = 0.0f);
