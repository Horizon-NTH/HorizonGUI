...
namespace hgui::kernel
{
	...
	public:
		...
		const Shader& set_dvec2(const std::string& uniformName, const dvec2& dvec2) const;
		const Shader& set_dvec3(const std::string& uniformName, const dvec3& dvec3) const;
		const Shader& set_dvec4(const std::string& uniformName, const dvec4& dvec4) const;
		const Shader& set_mat4(const std::string& uniformName, const glm::mat4& mat4) const;

	private:
		GLuint m_id;

		static GLuint load_shader(const std::string& vertexShaderCode, const std::string& fragmentShaderCode,
