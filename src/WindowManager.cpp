...
const std::shared_ptr<hgui::kernel::Window>& hgui::WindowManager::create(const std::string& windowName, const size& size, const point& position, const std::shared_ptr<kernel::Image>& icon, const std::shared_ptr<kernel::Monitor>& monitor, const std::initializer_list<std::pair<options, bool>>& options)
{
	if (m_windows.find(windowName) == m_windows.end())
	{
		m_windows[windowName] = std::make_shared<kernel::Window>(windowName, size, position, icon, monitor, options);
