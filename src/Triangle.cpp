...
hgui::kernel::shape::Triangle::Triangle(const point& firstVertex, const point& secondVertex, const point& thirdVertex, const color& color,
	const bool fill, const float thickness) : Shape(
		fill, thickness, color, std::array<hgui::point, 3>{firstVertex, secondVertex, thirdVertex})
{
	if (!m_shader)
	{
...
void hgui::kernel::shape::Triangle::draw(const hgui::point& canvasPosition, const hgui::size& canvasSize) const
{
	int width, height;
